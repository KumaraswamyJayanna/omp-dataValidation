...
class File_Report:
    ...
    def __init__(self, reportpath) -> None:
        ...
        self.timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        self.summary_report_path = REPORTPATH +f"/summary_report_{self.timestamp}.xlsx"
    ...
    def generate_report(self):
        ...
        output_file = self.summary_report_path
        ...
        extradata =[[],
                    ...
                    ["Data  from pipeline file which are Unable to compare are in the 'InPipelinenotInGT' sheet"],
                    ["Data  from Groundtruth file which are Unable to compare with pipeline are in the 'ExtrarowsInGT' sheet"]]
...
# copy the highlighted report path below
# report =""
